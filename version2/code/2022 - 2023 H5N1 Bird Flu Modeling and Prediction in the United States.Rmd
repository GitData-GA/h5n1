...
>
>Project Dashboard: https://h5n1.gd.edu.kg/version2/
>
>GitHub Repository: https://github.com/GitData-GA/h5n1/tree/master/version2
...
#######################################################################################
# Import Data
avg.temp = read.csv("https://h5n1.gd.edu.kg/version2/dataset/avg_temp.csv",
                    header = TRUE)
```
...
#######################################################################################
# Import Data
h5n1.cdc = read.csv("https://h5n1.gd.edu.kg/version2/dataset/h5n1_cdc.csv",
                    header = TRUE)
h5n1.cdc.clean = read.csv("https://h5n1.gd.edu.kg/version2/dataset/h5n1_cdc_clean.csv",
                          header = TRUE)
```
...
# save.as.svg(map.plot, "map_1", plot.name, 15, 100, 75, TRUE)
...
# save.as.svg(map.plot, "map_2", plot.name, 15, 100, 75, TRUE)
((map.plot[[1]] | map.plot[[2]] | map.plot[[3]])/
(map.plot[[4]] | map.plot[[5]] | map.plot[[6]]))
((map.plot[[7]] | map.plot[[8]] | map.plot[[9]])/
(map.plot[[10]] | map.plot[[11]] | map.plot[[12]]))
...
h5n1.cdc.clean1 = h5n1.cdc.clean1 %>%
  filter(cases > 0)
scatter.matrix = ggpairs(h5n1.cdc.clean1[, c(5, 4, 8, 9, 6, 7)]) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size = 5),
        axis.text.y = element_text(size = 5))
scatter.matrix
# Save as SVG
# scatter.matrix.exp = ggpairs(h5n1.cdc.clean1[, c(5, 4, 8, 9, 6, 7)])
# save.as.svg(scatter.matrix.exp, "scatter", "matrix", 1, 13, 13, FALSE,
...
# minimum lambda
cv.combine$lambda.min  # 1.717213e-05
# 1 stand error lambda
cv.combine$lambda.1se  # 0.004155875
# Plot lambda
plot(cv.combine)
# Coefficient using minimum lambda
...
x.test = model.matrix(as.factor(binary.case) ~ lat + lng + lat * lng + 
                    month.index + as.factor(type) + avg.temp, test)[,-1]
probabilities.combine = combine.model %>% predict(newx = x.test)
predicted.classes.combine = ifelse(probabilities.combine > 0.5, "infected", "uninfected")
# Model accuracy
